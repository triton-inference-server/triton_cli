...
from tensorrt_llm._utils import str_dtype_to_np, str_dtype_to_torch, torch_to_numpy
            ...
            if "input_layernorm.weight" in k:
                ...
                if use_weight_only:
                    ...
                    if not use_gemm_woq_plugin:
                        dst.value = (
                            torch.tensor(v).numpy().astype(str_dtype_to_np(dtype))
                        )
                    else:
                ...
                if use_weight_only:
                    ...
                    tensorrt_llm_llama.layers[
                        idx
                    ].mlp.experts_scale_2.value = torch_weight_scales
                else:
                ...
                if use_weight_only:
                    ...
                    tensorrt_llm_llama.layers[
                        idx
                    ].mlp.experts_scale_1.value = torch_weight_scales
                else:
                    dst.value = np.ascontiguousarray(v)
                ...
                tensorrt_llm_llama.layers[
                    idx
                ].mlp.router.weight.value = np.ascontiguousarray(v)
...
class QkvWeightHelper:
    ...
    def split_qkv_weights(self, layer_id):
        ...
        if not self.is_mha:
            ...
            fused_qkv = fused_qkv.reshape(3 * (q.shape[0] // self.tp_size), q.shape[1])
        return fused_qkv
...
def load_from_hf_checkpoint(
    ...
    assert (
        not tensorrt_llm_llama.moe_config.has_moe()
    ...
    for model_file in utils.iterate_shard_files(
        model_dir, rank=mapping.tp_rank, progress_bar=False
    ):
...
def load_from_meta_llama(
    ...
    def split_ckpt(ckpt, ranks_per_ckpt, ckpt_rank):
        ...
        for k, v in ckpt.items():
            d = 0
            if any(
                n in k for n in ["wo", "feed_forward.w2", "tok", "feed_forward.gate"]
            ):
                d = 1
            ...
            if "norm" in k or "rope" in k:  # no TP
                ...
                tmp = dup_kv_weight(v, tensorrt_llm_llama.num_kv_heads, mapping.tp_size)
                split_ckpt[k] = torch.split(tmp, tmp.shape[d] // ranks_per_ckpt, dim=d)[
                    ckpt_rank
                ].clone()
            elif (
                "experts" in k
                and moe_config.tp_mode == moe_config.ParallelismMode.EXPERT_PARALLEL
            ):
                rank_experts = mapping.ep_experts(moe_config.num_experts)
                ...
                split_ckpt[k] = torch.split(v, v.shape[d] // ranks_per_ckpt, dim=d)[
                    ckpt_rank
                ].clone()
        return split_ckpt
...
def load_from_gptq_llama(
    ...
    def process_and_assign_weight(mOp, v, tp_dim=-1):
        ...
        if not USE_UINT4_INPUT:
            ...
            qzeros_unpacked_int32 = (
                qzeros_unpacked_int32 + 16 * mask_negative - 16 * mask_positive
            )
        zeros_x_scales_fp16 = (
            -qzeros_unpacked_int32 + 8 * USE_UINT4_INPUT - USE_GPTQ_FOR_LLAMA
        ) * scales_fp16
    ...
    if mapping.is_last_pp_rank():
        tensorrt_llm_llama.lm_head.weight.value = (
            torch_split(v, 0).to(torch_dtype).cpu().numpy()
        )
    ...
    for l in layers_range:
        ...
        for suf in gptq_suffix_list:
            ...
            for comp in ["q", "k", "v"]:
                comp_part = load(
                    prefix + gptq_key_list[3] + comp + gptq_key_list[4] + suf
                )
...
def load_from_awq_llama(
    ...
    tensorrt_llm.logger.info("Loading weights from groupwise AWQ LLaMA checkpoint...")
    tik = time.time()
    ...
    if quant_ckpt_path.endswith(".pt"):
        ...
        group_size = (
            load("layers:0:attention:dense:weight").numel()
            // load("layers:0:attention:dense:weights_scaling_factor").numel()
        )
    else:
    ...
    def reSmooth_and_get_scale(weight, pre_quant_scale, avg_pre_quant_scale):
        ...
        if quant_ckpt_path.endswith("pt"):
            ...
            weight *= pre_quant_scale.repeat((n, 1)).transpose(1, 0).contiguous()
            weight /= avg_pre_quant_scale.repeat((n, 1)).transpose(1, 0).contiguous()
    ...
    def process_and_assign_qkv_weight(prefix, mOp):
        q_weight = load(
            prefix + "q" + awq_key_list[4] + awq_suffix_list[0]
        ).T.contiguous()
        k_weight = load(
            prefix + "k" + awq_key_list[4] + awq_suffix_list[0]
        ).T.contiguous()
        v_weight = load(
            prefix + "v" + awq_key_list[4] + awq_suffix_list[0]
        ).T.contiguous()
        ...
        q_pre_quant_scale = load(
            prefix + "q" + awq_key_list[4] + awq_suffix_list[2]
        ).reshape((1, dim_k))
        k_pre_quant_scale = load(
            prefix + "k" + awq_key_list[4] + awq_suffix_list[2]
        ).reshape((1, dim_k))
        v_pre_quant_scale = load(
            prefix + "v" + awq_key_list[4] + awq_suffix_list[2]
        ).reshape((1, dim_k))
        qkv_pre_quant_scale = (
    ...
    for l in layers_range:
        ...
        # 4.1 attention.qkv
        process_and_assign_qkv_weight(prefix + awq_key_list[3], layer.attention.qkv)
