...
def get_calib_dataloader(
    ...
    if data == "pileval":
        ...
        dataset = load_dataset(
            "cnn_dailymail", name="3.0.0", split="train", cache_dir=cache_dir
        )
        dataset = dataset["article"][:calib_size]
    else:
        raise NotImplementedError

    dataset_input_ids = tokenizer(
        dataset,
        return_tensors="pt",
        padding=True,
        truncation=True,
    ...
    calib_dataloader = DataLoader(
        dataset_input_ids, batch_size=batch_size, shuffle=False
    )
...
def get_tokenizer(ckpt_path, **kwargs):
    ...
    tokenizer = AutoTokenizer.from_pretrained(ckpt_path, padding_side="left", **kwargs)
...
def get_args():
    ...
    parser.add_argument(
        ...
        default="fp8",
        help="Quantization format. Currently only fp8 and int4_awq are supported. "
    ...
    parser.add_argument(
        "--cache_dir", type=str, default=None, help="Directory of dataset cache."
    )
...
def main():
    ...
    calib_dataloader = get_calib_dataloader(
        tokenizer=tokenizer, calib_size=args.calib_size, cache_dir=args.cache_dir
    )
    model = quantize_and_export(
        model,
