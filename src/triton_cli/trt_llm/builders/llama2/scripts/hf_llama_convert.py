...
def gpt_to_ft_name(orig_name):
    ...
    if weight_name == "self_attn.q_proj.weight":
        ...
        weight_name == "self_attn.k_proj.weight"
        or weight_name == "self_attn.v_proj.weight"
    ):
...
def hf_gpt_converter(args):
    ...
    model = LlamaForCausalLM.from_pretrained(
        args.in_file, torch_dtype=torch_dtype, device_map="auto", trust_remote_code=True
    )
    ...
    global_ft_weights = ["vocab_embedding.weight", "ln_f.weight", "lm_head.weight"]
    ...
    for name, param in model.named_parameters():
        ...
        if ft_name in global_ft_weights:
            ...
            qkv = (
                ...
                ft_name,
                llama_qkv_para.get(
                    name.replace(".weight", "").replace(".q_proj", ".qkv_proj")
                )
                .cpu()
                .numpy()
                .astype(storage_type),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out-dir", "-o", type=str, help="file name of output directory", required=True
    )
    parser.add_argument(
        "--in-file",
        "-i",
    ...
    assert (
        args.calibrate_kv_cache or args.smoothquant
