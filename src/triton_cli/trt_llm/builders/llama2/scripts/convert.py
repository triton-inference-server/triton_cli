...
def generate_int8(weights, act_range, is_qkv=False, multi_query_mode=False):
    ...
    if is_qkv and multi_query_mode:
        ...
        scale_w_quant_orig_t_expand[
            hidden_dim : hidden_dim + kv_dim
        ] = scale_w_quant_orig_t[1]
...
def save_multi_query_mode_qkv_int8(
    val, dir, base_key, saved_key, factor, rank, local_dim, head_size
):
...
def write_int8(
    vals, dir, base_key, split_dim, i, factor, is_qkv=False, multi_query_mode=False
):
    saved_keys_once = [
        "scale_x_orig_quant",
        "scale_w_quant_orig",
    ...
    if is_qkv and multi_query_mode:
        ...
        save_multi_query_mode_qkv_int8(
            vals["weight.int8"],
            dir,
            base_key,
            "weight.int8",
        ...
        save_split(
            np.split(vals["weight.int8"], factor, axis=split_dim),
            dir,
        ...
        save_split(
            np.split(vals["weight.int8.col"], factor, axis=split_dim),
            dir,
        ...
        if split_dim == -1:
            ...
            if is_qkv:
                save_split(
                    np.split(vals["scale_y_accum_quant"], factor, axis=split_dim),
                    dir,
                    f"{base_key}.scale_y_accum_quant",
                    i,
                    factor,
                )
                save_split(
                    np.split(vals["scale_w_quant_orig"], factor, axis=split_dim),
                    dir,
                    f"{base_key}.scale_w_quant_orig",
                    i,
                    factor,
                )
                saved_keys_once = ["scale_x_orig_quant", "scale_y_quant_orig"]
...
def split_and_save_weight(i, saved_dir, factor, key, val, act_range, config):
    ...
    if (
        ...
        or "post_layernorm.weight" in key
        ...
        if multi_query_mode:
            ...
            w_q, w_k, w_v = np.split(val, [local_dim, local_dim + head_size], axis=-1)
            w_q_split = np.split(w_q, factor, axis=-1)
            w_k_split = np.split(w_k, factor, axis=-1)
            w_v_split = np.split(w_v, factor, axis=-1)
            split_vals = [
                np.concatenate((w_q_split[ii], w_k_split[ii], w_v_split[ii]), axis=-1)
                for ii in range(factor)
            ]
            split_dim = -1
        else:
