...
def parse_arguments(args):
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Enable horizontal fusion in GatedMLP, reduces layer input traffic and potentially improves performance. "
    ...
    if args.model_dir is not None:
        ...
        args.vocab_size = (
            ...
            if hf_config.vocab_size is not None
        ...
        args.moe_num_experts = getattr(
            hf_config, "num_local_experts", args.moe_num_experts
        )
    ...
    lora_config = LoraConfig.from_hf(args.hf_lora_dir, hf_modules_to_trtllm_modules)
...
def build_rank_engine(
    ...
    tensorrt_llm_llama = quantize_model(
        tensorrt_llm_llama, args.quant_mode, **quantize_kwargs
    )
    if args.per_group:
        load_func = (
            load_from_awq_llama
        ...
        if not args.load_by_shard:
            hf_model = (
                LlamaForCausalLM if args.model_type != "mixtral" else MixtralForCausalLM
            )
...
def build(rank, args):
    ...
    for cur_rank in range(args.world_size):
        ...
        local_num_kv_heads = (args.n_kv_head + args.world_size - 1) // args.world_size
        ...
        tok = time.time()
        t = time.strftime("%H:%M:%S", time.gmtime(tok - tik))
        logger.info(f"Rank {cur_rank} Engine build time: {t} - {tok - tik} (sec)")
