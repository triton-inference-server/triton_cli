...
def parse_ft_config(ini_file):
    ...
    rotary_scaling_type = gpt_config.get("gpt", "rotary_scaling_type", fallback=None)
    ...
    multi_query_mode = gpt_config.getboolean("gpt", "multi_query_mode", fallback=False)
...
def load_from_ft(
    ...
    if not share_embedding_table:
        ...
        if lm_head_weight is None:
            lm_head_weight = fromfile(dir_path, "model.wte.bin", [vocab_size, n_embd])
        if vocab_size % tensor_parallel != 0:
            # padding
    ...
    for i in range(n_layer):
        ...
        dst.value = fromfile(
            dir_path, "model.layers." + str(i) + ".post_attention_layernorm.weight.bin"
        )

        dst = gpt_layer.post_layernorm.bias
        dst.value = fromfile(
            dir_path, "model.layers." + str(i) + ".post_attention_layernorm.bias.bin"
        )
        t = fromfile(
            dir_path,
        ...
        if enable_fp8_qdq:
            ...
            tensorrt_llm_gpt.layers[i].mlp.fc.weights_scaling_factor.value = np.array(
        ...
        if enable_fp8_qdq:
            ...
            tensorrt_llm_gpt.layers[i].mlp.proj.weights_scaling_factor.value = np.array(
        ...
        if "wte.weight" in k:
            ...
            if "ln_1.weight" in k:
                ...
                if multi_query_mode:
                    ...
                    w_qkv = v.reshape(hidden_size + 2 * head_size, 3, hidden_size)
                    ...
                    v = v.transpose()
                    dst = tensorrt_llm_gpt.layers[idx].attention.qkv.weight
                    dst.value = np.ascontiguousarray(split(v, tensor_parallel, rank))
            elif "attn.c_attn.bias" in k:
                if multi_query_mode:
                    v.reshape(hidden_size + 2 * head_size, 3)
                    bias_q, bias_kv = np.split(w_qkv, [hidden_size, 2 * head_size])
                    bias_q = split(bias_q, tensor_parallel, rank)
                    dst = tensorrt_llm_gpt.layers[idx].attention.qkv.bias
                    dst.value = np.ascontiguousarray(np.concatenate(bias_q, bias_kv))
                else:
                    dst = tensorrt_llm_gpt.layers[idx].attention.qkv.bias
                    dst.value = np.ascontiguousarray(split(v, tensor_parallel, rank))
                ...
                dst.value = np.ascontiguousarray(split(v, tensor_parallel, rank, dim=1))
                ...
                v = v.transpose()
                tensorrt_llm_gpt.layers[idx].mlp.fc.weight.value = np.ascontiguousarray(
                    split(v, tensor_parallel, rank)
                )
            elif "mlp.c_fc.bias" in k:
                tensorrt_llm_gpt.layers[idx].mlp.fc.bias.value = np.ascontiguousarray(
                    split(v, tensor_parallel, rank)
                )
