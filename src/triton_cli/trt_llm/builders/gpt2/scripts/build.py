...
from triton_cli.trt_llm.builders.gpt2.scripts.weight import (
    load_from_ft,
    parse_ft_config,
    check_embedding_share,
)  # isort:skip
...
def parse_arguments(args):
    ...
    parser.add_argument(
        "--logits_dtype", type=str, default="float32", choices=["float16", "float32"]
    )
    parser.add_argument(
        "--timing_cache",
        type=str,
    ...
    parser.add_argument(
        ...
        default=False,
        choices=["float16", "float32", "bfloat16"],
        help="Activates attention plugin. You can specify the plugin dtype or leave blank to use the model dtype.",
    )
    parser.add_argument(
        "--use_gemm_plugin",
        nargs="?",
        ...
        default=False,
        choices=["float16", "float32", "bfloat16"],
        help="Activates GEMM plugin. You can specify the plugin dtype or leave blank to use the model dtype.",
    )
    parser.add_argument(
        ...
        default=False,
        choices=["float16", "float32", "bfloat16"],
        help="Activates layernorm plugin. You can specify the plugin dtype or leave blank to use the model dtype.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether this model uses multi-query attention mechanism (default: False)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Seed to use when initializing the random number generator for torch.",
    )
    parser.add_argument(
        "--paged_kv_cache",
        action="store_true",
    ...
    parser.add_argument("--gather_all_token_logits", action="store_true", default=False)
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="By default, we use dtype for KV cache. fp8_kv_cache chooses fp8 quantization for KV",
    )
    parser.add_argument(
        "--max_num_tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum lengths of draft tokens for speculative decoding target model.",
    )
    parser.add_argument(
        "--use_paged_context_fmha",
        action="store_true",
        help="Activates paged context FMHA. This mode of the context FMHA is required for chunked context, speculative decoding and reuse of KV cache blocks. Context FMHA performance is worse when this mode is on.",
    )
    parser.add_argument(
        "--use_context_fmha_for_generation",
        action="store_true",
        help="Activates context FMHA for generation phase instead of MMHA. Use only for testing and debug.",
    )
    parser.add_argument(
        "--lora_target_modules",
        nargs="+",
    ...
    if args.rotary_scaling is not None:
        ...
        ), "RoPE scaling is only supported through GPT attention plugin."
        rotary_scaling = {
...
def build_rank_engine(
    ...
    if args.model_dir is not None:
        ...
        load_from_ft(
            ...
            scaling_factors=gpt_dummy_fp8_scaling_factors if args.enable_fp8 else None,
        )
...
def build(rank, args):
    ...
    timing_cache_file = (
        args.timing_cache if args.timing_cache else args.output_dir / "model.cache"
    )
    ...
    for cur_rank in range(args.world_size):
        ...
        local_num_kv_heads = (num_kv_heads + args.world_size - 1) // args.world_size
        kv_dtype = str_dtype_to_trt(args.dtype)
