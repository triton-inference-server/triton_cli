...
from tensorrt_llm.runtime.lora_manager import LoraConfig, get_all_nemo_lora_weights
...
def lora_convert(out_dir, lora_config, lora_weights, customization_id, precision):
    ...
    for layer_id in range(num_layers):
        ...
        save_val(
            torch_to_numpy(
                linear_in_weight.transpose(1, 0)
                .contiguous()
                .to(dtype=str_dtype_to_torch(precision))
            ),
            saved_dir,
        ...
        save_val(
            torch_to_numpy(
                linear_out_weight.transpose(1, 0)
                .contiguous()
                .to(dtype=str_dtype_to_torch(precision))
            ),
            saved_dir,
...
def lora_convert_cpp_runtime(out_dir, lora_config, lora_weights, precision="float16"):
    saved_dir = Path(out_dir)
    saved_dir.mkdir(parents=True, exist_ok=True)
    num_layers = int(lora_config["num_layers"])
    weights = []
    ...
    save_val(all_weights, saved_dir, "lora_weights", tp_num=None, write_npy=True)
...
def main(args):
    ...
    with tempfile.TemporaryDirectory() as prompt_out_dir:
        ...
        LOGGER.info(
            "Spent %s (h:m:s) to unpack NeMo prompt archive",
    ...
    if args.write_cpp_runtime_tensors:
        lora_convert_cpp_runtime(
            args.out_dir, prompt_config, prompt_weights, precision=args.storage_type
        )
    else:
        lora_convert(
            args.out_dir,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--write-cpp-runtime-tensors", action="store_true", default=False
    )
    parser.add_argument(
        "--storage-type",
        type=str,
