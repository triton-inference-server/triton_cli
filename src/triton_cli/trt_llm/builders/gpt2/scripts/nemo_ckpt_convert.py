...
def convert_checkpoint(unpacked_checkpoints_dir: UnpackedNemoCheckpointDir, args):
    ...
    map_location_fn = (
        gpu_map_location if args.load_checkpoints_on_gpu else cpu_map_location
    )
    ...
    ini_config = nemo_config_to_ini_config(
        nemo_model_config,
        *get_eos_bos_ids_from_tokenizer_config(tokenizer_config),
        vocab_size,
        args.storage_type,
    )
...
def main():
    ...
    parser.add_argument(
        "--out-dir", "-o", type=Path, help="path to output directory", required=True
    )
    parser.add_argument(
        "--in-file",
        "-i",
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if args.in_file.is_dir():
            ...
            LOGGER.info(
                "Spent %s (h:m:s) to unpack NeMo archive",
        ...
        LOGGER.info(
            "Spent %s (h:m:s) to convert the model",
            datetime.datetime.now() - start_time,
        )
