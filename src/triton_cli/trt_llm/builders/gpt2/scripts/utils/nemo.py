...
def set_parameter_from_config(
    params: Dict[str, Any], nemo_config: Dict[str, Any], param_name: str
) -> None:
...
def extract_rotary_parameters_from_nemo_config(
    ...
    set_parameter_from_config(params, nemo_config, "seq_len_interpolation_factor")
...
def add_rotary_parameters_to_ini_config(
    config: configparser.ConfigParser, rotary_parameters: NemoRotaryEmbeddingParameters
) -> None:
    if rotary_parameters.position_embedding_type == "rope":
        if (
            rotary_parameters.rotary_percentage > 1.0
            or rotary_parameters.rotary_percentage <= 0.0
        ):
            raise ValueError(
...
def nemo_config_to_ini_config(
    ...
    gpt_model_config = nemo_to_gpt_config(nemo_model_config, vocab_size, eos_id, bos_id)
