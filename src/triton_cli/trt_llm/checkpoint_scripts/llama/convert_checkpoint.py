...
def parse_arguments(args=None):
    ...
    parser.add_argument(
        "--storage-type", "-t", type=str, default="fp32", choices=["fp32", "fp16"]
    )
    parser.add_argument(
        "--dataset-cache-dir",
        type=str,
...
def get_tllm_linear_sq_weight(
    ...
    if smoother_value is not None:
        ...
        results[prefix + "smoother"] = (
            cur_smoother_value.reshape(smoother_shape).clone().to(torch.float32)
        )
...
def convert_hf_llama(
    ...
    for layer in layers_range:
        ...
        if int8_kv_cache:
            ...
            kv_cache_weights[
                tllm_prex + "attention.kv_cache_scaling_factor"
        ...
        if moe_config and moe_config.has_moe():
            ...
            v = split(
                moe_experts_gate_weights, mapping.tp_size, mapping.tp_rank, dim=-1
            )
    ...
    if mapping.is_last_pp_rank():
        ...
        if vocab_size % mapping.tp_size != 0:
            ...
            lm_head_weights = torch.from_numpy(
                np.pad(
                    lm_head_weights.detach().cpu().numpy(),
                    ((0, pad_width), (0, 0)),
                    "constant",
                    constant_values=0,
                )
...
def main(args=None):
    ...
    lora_config = LoraConfig.from_hf(
        args.hf_lora_dir, hf_modules_to_trtllm_modules, trtllm_modules_to_hf_modules
    )
    ...
    if args.model_dir is not None:
        ...
        if args.smoothquant is not None or args.int8_kv_cache:
            ...
            dataset = load_dataset(
                "ccdv/cnn_dailymail", "3.0.0", cache_dir=args.dataset_cache_dir
            )
            ...
            if args.smoothquant is not None:
                smooth_llama_model(
                    model, act_range, args.smoothquant, llama_qkv_para, llama_smoother
                )
    convert_args = {
        "hf_model": model,
        "act_range": act_range,
