...
args = {
    ...
    "moe_config": MoeConfig(
        num_experts=0,
        top_k=0,
        tp_mode=MoeConfig.ParallelismMode.TENSOR_PARALLEL,
...
class GPTBuilder:
    def __init__(self, engine_output_path: Path):
        global args
        args["output_dir"] = engine_output_path
    ...
    def build_rank_engine(
        ...
        if args["use_embedding_sharing"]:
            if args["world_size"] > 1:
                if (
                    ...
                    and args["embedding_sharding_dim"] == 0
                    and args["use_parallel_embedding"]
                ):
                    share_embedding_table = self.check_embedding_share(
                        args["model_dir"]
                    )
            else:
        ...
        if share_embedding_table:
            logger.info("Engine will share embedding and language modeling weights.")
        ...
        tensorrt_llm_gpt = tensorrt_llm.models.GPTLMHeadModel(
            ...
            mapping=Mapping(
                world_size=args["world_size"], rank=rank, tp_size=args["world_size"]
            ),  # TP only
            apply_query_key_layer_scaling=builder_config.apply_query_key_layer_scaling,
        ...
        if args["use_gemm_plugin"]:
            if not args["enable_fp8"]:
                ...
                logger.info("Gemm plugin does not support FP8. Disabled Gemm plugin.")
        if args["use_layernorm_plugin"]:
            network.plugin_config.set_layernorm_plugin(
                dtype=args["use_layernorm_plugin"]
            )
        assert not (
            args["enable_context_fmha"] and args["enable_context_fmha_fp32_acc"]
        )
        ...
        if args["world_size"] > 1:
            network.plugin_config.set_nccl_plugin(
                args["dtype"], args["use_custom_all_reduce"]
            )
    ...
    def build(self, rank=0):
        ...
        timing_cache_file = (
            ...
            else args["output_dir"] / "model.cache"
        )
        ...
        for cur_rank in range(args["world_size"]):
            ...
            num_kv_heads = 1 if args["multi_query_mode"] else args["n_head"]
            # FIXME DLIS-6056: Currently, IFB engines built with tensorrt_llm v0.7.0 build and launch successfully but crash
            ...
            engine_name = self.get_engine_name(
                MODEL_NAME, args["dtype"], args["world_size"], cur_rank
            )
            engine = self.build_rank_engine(
                builder, builder_config, engine_name, cur_rank
            )
    ...
    def parse_ft_config(self, ini_file):
        ...
        do_layer_norm_before = gpt_config.getboolean(
            "gpt", "do_layer_norm_before", fallback=True
        )
        rotary_base = gpt_config.getfloat("gpt", "rotary_base", fallback=None)
        rotary_scaling_type = gpt_config.get(
            "gpt", "rotary_scaling_type", fallback=None
        )
        rotary_scaling_factor = gpt_config.get(
            "gpt", "rotary_scaling_factor", fallback=None
        )
        ...
        multi_query_mode = gpt_config.getboolean(
            "gpt", "multi_query_mode", fallback=False
        )
        prompt_num_tasks = gpt_config.getint("gpt", "prompt_num_tasks", fallback=0)
        prompt_max_vocab_size = gpt_config.getint(
            "gpt", "prompt_max_vocab_size", fallback=0
        )
        return {
            "n_embd": n_embd,
            "n_head": n_head,
    ...
    def load_from_ft(
        self,
        tensorrt_llm_gpt: GPTLMHeadModel,
        dir_path,
        rank=0,
        tensor_parallel=1,
        dtype="float32",
        ...
        def set_smoothquant_scale_factors(
            ...
            if per_tok_dyn:
                ...
                t = fromfile(
                    dir_path, f"{basename}scale_x_orig_quant.bin", [1], np.float32
                )
        ...
        vocab_embedding_weight = fromfile(
            dir_path, "model.wte.bin", [vocab_size, n_embd]
        )
        if not use_parallel_embedding:
            tensorrt_llm_gpt.embedding.vocab_embedding.weight.value = (
            ...
            tensorrt_llm_gpt.embedding.vocab_embedding.weight.value = (
                np.ascontiguousarray(
                    self.split(
                        vocab_embedding_weight, tensor_parallel, rank, dim=sharding_dim
                    )
                )
            )
        ...
        if not share_embedding_table:
            lm_head_weight = fromfile(
                dir_path, "model.lm_head.weight.bin", [vocab_size, n_embd]
            )
            ...
            if vocab_size % tensor_parallel != 0:
                # padding
                vocab_size_padded = (
                    tensorrt_llm_gpt.lm_head.out_features * tensor_parallel
                )
                pad_width = vocab_size_padded - vocab_size
        ...
        for i in range(n_layer):
            c_attn_out_dim = (
                (3 * n_embd // tensor_parallel)
                if not multi_query_mode
                else (n_embd // tensor_parallel + (n_embd // n_head) * 2)
            )
            ...
            if enable_fp8_qdq:
                tensorrt_llm_gpt.layers[
                    i
                ].attention.qkv.activation_scaling_factor.value = np.array(
                ...
                tensorrt_llm_gpt.layers[
                    i
                ].attention.kv_orig_quant_scale.value = np.array(
                    [scaling_factors["qkv_output"][i]], dtype=np.float32
                )
                tensorrt_llm_gpt.layers[
                    i
                ].attention.kv_quant_orig_scale.value = np.array(
            ...
            if use_smooth_quant:
                dst.value = np.ascontiguousarray(np.transpose(t, [1, 0]))
                dense_scale = getattr(
                    gpt_layer.attention, "quantization_scaling_factor", None
                )
                set_smoothquant_scale_factors(
                    gpt_layer.attention.dense,
                    dense_scale,
                    dir_path,
            ...
            if enable_fp8_qdq:
                tensorrt_llm_gpt.layers[
                    i
                ].attention.dense.activation_scaling_factor.value = np.array(
                    [scaling_factors["dense_act"][i]], dtype=fake_fp8_sf_dt
                )
                tensorrt_llm_gpt.layers[
                    i
                ].attention.dense.weights_scaling_factor.value = np.array(
            ...
            if is_gated_activation(hidden_act):
                t = fromfile(
                    ...
                    "model.layers."
                    + str(i)
                    + ".mlp.dense_h_to_4h.gate.weight."
                    + str(rank)
                    + ".bin",
                    [n_embd, inter_size // tensor_parallel],
                )
                tensorrt_llm_gpt.layers[i].mlp.gate.weight.value = np.ascontiguousarray(
                    np.transpose(t, [1, 0])
                )
            if enable_fp8_qdq:
                tensorrt_llm_gpt.layers[
                    i
                ].mlp.fc.activation_scaling_factor.value = np.array(
                    [scaling_factors["fc_act"][i]], dtype=fake_fp8_sf_dt
                )
                tensorrt_llm_gpt.layers[
                    i
                ].mlp.fc.weights_scaling_factor.value = np.array(
            ...
            if use_smooth_quant:
                tensorrt_llm_gpt.layers[i].mlp.proj.weight.value = np.ascontiguousarray(
                    np.transpose(t, [1, 0])
                )
                proj_scale = getattr(gpt_layer.mlp, "quantization_scaling_factor", None)
                set_smoothquant_scale_factors(
                    gpt_layer.mlp.proj,
                    proj_scale,
                    dir_path,
            ...
            if enable_fp8_qdq:
                tensorrt_llm_gpt.layers[
                    i
                ].mlp.proj.activation_scaling_factor.value = np.array(
                    [scaling_factors["proj_act"][i]], dtype=fake_fp8_sf_dt
                )
                tensorrt_llm_gpt.layers[
                    i
                ].mlp.proj.weights_scaling_factor.value = np.array(
    ...
    def hf_gpt_converter(self):
        ...
        multi_query_mode = (
            True if converter_args["model"] in ["santacoder", "starcoder"] else False
        )
        ...
        act_range = {}
        if (
            converter_args["smoothquant"] is not None
            or converter_args["calibrate_kv_cache"]
        ):
    ...
    def smooth_gemm(
        ...
        self.apply_smoothing(
            scales, gemm_weights, layernorm_weights, layernorm_bias, orig_dtype
        )
    ...
    def capture_activation_range(
        self, model, tokenizer, dataset, num_samples=512, seq_len=512
    ):
        model.eval()
    ...
    def write_int8(
        self, vals, dir, base_key, split_dim, tp_rank, split_factor, kv_cache_only=False
    ):
    ...
    def split_and_save_weight(
        ...
        if (
            ...
            if act_range is not None and int8_outputs == "all":
                ...
                vals_i8 = self.generate_int8(
                    val, act_range, multi_query_mode=multi_query_mode
                )
                self.write_int8(
                    vals_i8, saved_dir, base_key, cat_dim, tp_rank, split_factor
                )
